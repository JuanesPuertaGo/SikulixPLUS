...
//</editor-fold>
  ...
  protected static class Finder2 {
    ...
    // fallback scale factors tried when the plain (1.0x) search does not reach
    // MinSimilarity - covers common Windows/macOS display-scaling percentages
    // (125%, 150%, 175%, 200%, and their inverses) so a pattern captured at one
    // DPI/screen-scale still matches a screen running a different one
    private static final double[] scaleFallbackFactors =
        new double[]{0.57, 0.67, 0.75, 0.8, 0.9, 1.1, 1.25, 1.33, 1.5, 1.75, 2.0};
    // a pattern resized below this many pixels per side gets too little distinctive
    // detail left to match reliably, risking false positives elsewhere on screen
    private static final int scaleFallbackMinPixels = 20;
    // scale fallback searches a much bigger space (many resize levels) than the plain
    // search, so a small margin above MinSimilarity is required before trusting a hit -
    // otherwise a coincidental low-confidence match at some scale gets accepted
    private static final double scaleFallbackScoreMargin = 0.05;
    ...
    private FindResult2 doFindImage() {
      ...
      // brightness fallback first: same geometry as the plain search (no resize), so it
      // carries no extra risk of matching the wrong element - try it before the
      // geometrically riskier scale fallback
      if (findResult == null && !findInput.isFindAll()) {
        findResult = findWithBrightnessFallback(findInput, findWhere);
      }
      if (findResult == null && !findInput.isFindAll()) {
        findResult = findWithScaleFallback(findInput, findWhere);
      }
    ...
    /**
     * Retried only when the plain search above did not reach MinSimilarity: resizes the
     * pattern across common DPI/screen-scale factors and reruns the match, to cope with a
     * reference image captured at a different display scale than the current screen.
     */
    private FindResult2 findWithScaleFallback(FindInput2 findInput, Mat findWhere) {
      if (findInput.hasMask() || findInput.isExact()) {
        return null;
      }
      Mat originalTarget = findInput.getTarget();
      Size origSize = originalTarget.size();
      double wantedScore = findInput.getScore();
      double bestScore = wantedScore + scaleFallbackScoreMargin;
      double bestScale = 0;
      Mat bestResult = null;
      Mat bestTarget = null;
      for (double scaleFactor : scaleFallbackFactors) {
        int newW = (int) Math.round(origSize.width * scaleFactor);
        int newH = (int) Math.round(origSize.height * scaleFactor);
        if (newW < scaleFallbackMinPixels || newH < scaleFallbackMinPixels
            || newW > findWhere.cols() || newH > findWhere.rows()) {
          continue;
        }
        Mat scaledTarget = new Mat();
        Imgproc.resize(originalTarget, scaledTarget, new Size(newW, newH), 0, 0, Imgproc.INTER_LINEAR);
        Mat scaledResult = doFindMatch(scaledTarget, findWhere, findInput);
        Core.MinMaxLocResult mm = Core.minMaxLoc(scaledResult);
        if (mm.maxVal > bestScore) {
          bestScore = mm.maxVal;
          bestScale = scaleFactor;
          bestResult = scaledResult;
          bestTarget = scaledTarget;
        }
      }
      if (bestResult == null) {
        return null;
      }
      log.trace("doFindImage: found via scale-fallback %.2fx: %%%.2f (?%%%.2f)",
          bestScale, bestScore * 100, wantedScore * 100);
      // the match rectangle reported to the caller must reflect the scale that actually
      // matched, so the click point lands on the real (scaled) element, not the original size
      findInput.applyScaledTarget(bestTarget);
      return new FindResult2(bestResult, findInput);
    }

    /**
     * Retried only when the plain (and scale-fallback) search still did not reach
     * MinSimilarity: normalizes local contrast/illumination (CLAHE) on both the screen
     * region and the pattern before matching, to cope with brightness/backlight changes
     * that a linear-normalized correlation alone does not fully absorb.
     */
    private FindResult2 findWithBrightnessFallback(FindInput2 findInput, Mat findWhere) {
      double wantedScore = findInput.getScore();
      Mat grayWhere = new Mat();
      Mat grayTarget = new Mat();
      Imgproc.cvtColor(findWhere, grayWhere, Imgproc.COLOR_BGR2GRAY);
      Imgproc.cvtColor(findInput.getTarget(), grayTarget, Imgproc.COLOR_BGR2GRAY);
      CLAHE clahe = Imgproc.createCLAHE(3.0, new Size(8, 8));
      Mat eqWhere = new Mat();
      Mat eqTarget = new Mat();
      clahe.apply(grayWhere, eqWhere);
      clahe.apply(grayTarget, eqTarget);
      Mat eqResult = new Mat();
      Imgproc.matchTemplate(eqWhere, eqTarget, eqResult, Imgproc.TM_CCOEFF_NORMED);
      Core.MinMaxLocResult mm = Core.minMaxLoc(eqResult);
      if (mm.maxVal <= wantedScore) {
        return null;
      }
      log.trace("doFindImage: found via brightness-fallback: %%%.2f (?%%%.2f)", mm.maxVal * 100, wantedScore * 100);
      return new FindResult2(eqResult, findInput);
    }
  ...
  private static class FindInput2 {
    ...
    /**
     * Used only by the scale-fallback search: replaces the target with the resized
     * version that actually matched, so the reported Match rectangle (and therefore the
     * click point) reflects the real on-screen size instead of the original pattern size.
     */
    protected void applyScaledTarget(Mat scaledTarget) {
      this.targetBGR = scaledTarget;
      this.target = scaledTarget;
    }
